...
export const getNewSeasonAnimes = async (): Promise<Anime[] | any> => {
    const res = await url.get('/seasons/now?filter=tv');
    return res.data;
}
...
export const getTrendingAnimes = async (): Promise<Anime[] | any> => {
    const res = await url.get('/top/anime?type=tv&filter=airing');
    return res.data;
}


/* Request de apenas 1 anime usando id como filtro */
export const getSingleAnime = async (id: string): Promise<Anime | any> => {
    const res = await url.get(`/anime/${id}`);
    return res.data;
}


/* Request search */
export const getSearchAnime = async (name: string): Promise<Anime | any> => {
    const res = await url.get(`/anime?type=tv&q=${name}`);
    return res.data;
}